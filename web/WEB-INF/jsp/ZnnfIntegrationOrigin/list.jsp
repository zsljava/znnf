						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfIntegrationOrigin/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">ORIGIN_MODE</th>
														<th class="sorting_disabled">ORIGIN_TYPE</th>
														<th class="sorting_disabled">ORIGIN_CONSUME</th>
														<th class="sorting_disabled">ORIGIN_TIME</th>
														<th class="sorting_disabled">ORIGIN_REMARK</th>
														<th class="sorting_disabled">ORIGIN_CREATE_NAME</th>
														<th class="sorting_disabled">ORIGIN_CREATE_TIME</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfIntegrationOrigins}" var="znnfIntegrationOrigin">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfIntegrationOrigin.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfIntegrationOrigin.origin_mode}</td>
															<td>${znnfIntegrationOrigin.origin_type}</td>
															<td>${znnfIntegrationOrigin.origin_consume}</td>
															<td>${znnfIntegrationOrigin.origin_time}</td>
															<td>${znnfIntegrationOrigin.origin_remark}</td>
															<td>${znnfIntegrationOrigin.origin_create_name}</td>
															<td>${znnfIntegrationOrigin.origin_create_time}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfIntegrationOrigin/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfIntegrationOrigin/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfIntegrationOrigin/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfIntegrationOrigin/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
