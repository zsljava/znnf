						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfManager/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">MANAGER_NAME</th>
														<th class="sorting_disabled">MANAGER_PASSWORD</th>
														<th class="sorting_disabled">MANAGER_STATUS</th>
														<th class="sorting_disabled">MANAGER_CREATE_NAME</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfManagers}" var="znnfManager">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfManager.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfManager.manager_name}</td>
															<td>${znnfManager.manager_password}</td>
															<td>${znnfManager.manager_status}</td>
															<td>${znnfManager.manager_create_name}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManager/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManager/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManager/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManager/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
