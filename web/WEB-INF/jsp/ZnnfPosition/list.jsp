						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfPosition/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">POSITION_NAME</th>
														<th class="sorting_disabled">POSITION_LEVEL</th>
														<th class="sorting_disabled">POSITION_DESC</th>
														<th class="sorting_disabled">POSITION_STATUS</th>
														<th class="sorting_disabled">POSITION_TYPE</th>
														<th class="sorting_disabled">POSITOIN_REMARK</th>
														<th class="sorting_disabled">POSITION_CREATE_NAME</th>
														<th class="sorting_disabled">POSITION_CREATE_TIME</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfPositions}" var="znnfPosition">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfPosition.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfPosition.position_name}</td>
															<td>${znnfPosition.position_level}</td>
															<td>${znnfPosition.position_desc}</td>
															<td>${znnfPosition.position_status}</td>
															<td>${znnfPosition.position_type}</td>
															<td>${znnfPosition.positoin_remark}</td>
															<td>${znnfPosition.position_create_name}</td>
															<td>${znnfPosition.position_create_time}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfPosition/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfPosition/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfPosition/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfPosition/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
