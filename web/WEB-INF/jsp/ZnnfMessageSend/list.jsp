						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfMessageSend/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">SEND_CONTENT</th>
														<th class="sorting_disabled">SEND_CREATE_NAME</th>
														<th class="sorting_disabled">SEND_CREATE_TIME</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfMessageSends}" var="znnfMessageSend">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfMessageSend.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfMessageSend.send_content}</td>
															<td>${znnfMessageSend.send_create_name}</td>
															<td>${znnfMessageSend.send_create_time}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageSend/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageSend/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageSend/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageSend/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
