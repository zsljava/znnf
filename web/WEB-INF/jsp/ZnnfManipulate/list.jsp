						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfManipulate/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">MANIPULATE_NAME</th>
														<th class="sorting_disabled">MANIPULATE_TIME</th>
														<th class="sorting_disabled">MANIPULATE_DESC</th>
														<th class="sorting_disabled">MANIPULATE_IP</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfManipulates}" var="znnfManipulate">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfManipulate.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfManipulate.manipulate_name}</td>
															<td>${znnfManipulate.manipulate_time}</td>
															<td>${znnfManipulate.manipulate_desc}</td>
															<td>${znnfManipulate.manipulate_IP}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManipulate/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManipulate/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManipulate/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfManipulate/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
