						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfMember/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">MEMBER_NAME</th>
														<th class="sorting_disabled">MEMBER_NUMBER</th>
														<th class="sorting_disabled">MEMBER_TYPE</th>
														<th class="sorting_disabled">MEMBER_ID_TYPE</th>
														<th class="sorting_disabled">MEMBER_ID_NUMBER</th>
														<th class="sorting_disabled">MEMBER_PHONE</th>
														<th class="sorting_disabled">MEMBER_STATUS</th>
														<th class="sorting_disabled">MEMBER_ID_PHOTO</th>
														<th class="sorting_disabled">MEMBER_LANDLINE_NUMBER</th>
														<th class="sorting_disabled">MEMBER_ADDRESS</th>
														<th class="sorting_disabled">MEMBER_ZIPCODE</th>
														<th class="sorting_disabled">MEMBER_EMAIL</th>
														<th class="sorting_disabled">MEMBER_RECOMMENDED</th>
														<th class="sorting_disabled">MEMBER_BANK</th>
														<th class="sorting_disabled">MEMBER_BANK_CARD_NUMBER</th>
														<th class="sorting_disabled">MENBER_CROP_NAME</th>
														<th class="sorting_disabled">MEMBER_ACREAGE</th>
														<th class="sorting_disabled">MEMBER_CULTIVATE_POSITION</th>
														<th class="sorting_disabled">MEMBER_SOIL_FERTILITY</th>
														<th class="sorting_disabled">MEMBER_CROP_HISTORICAL_PRODUCTION</th>
														<th class="sorting_disabled">MEMBER_FERTILIZER_COSTS</th>
														<th class="sorting_disabled">MEMBER_COMPREHENSIVE_COST</th>
														<th class="sorting_disabled">MEMBER_REMARK</th>
														<th class="sorting_disabled">MEMBER_UPDATE_NAME</th>
														<th class="sorting_disabled">MEMBER_UPDATE_TIME</th>
														<th class="sorting_disabled">COMMISSIONER_ID</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfMembers}" var="znnfMember">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfMember.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfMember.member_name}</td>
															<td>${znnfMember.member_number}</td>
															<td>${znnfMember.member_type}</td>
															<td>${znnfMember.member_id_type}</td>
															<td>${znnfMember.member_id_number}</td>
															<td>${znnfMember.member_phone}</td>
															<td>${znnfMember.member_status}</td>
															<td>${znnfMember.member_id_photo}</td>
															<td>${znnfMember.member_landline_number}</td>
															<td>${znnfMember.member_address}</td>
															<td>${znnfMember.member_zipcode}</td>
															<td>${znnfMember.member_email}</td>
															<td>${znnfMember.member_recommended}</td>
															<td>${znnfMember.member_bank}</td>
															<td>${znnfMember.member_bank_card_number}</td>
															<td>${znnfMember.menber_crop_name}</td>
															<td>${znnfMember.member_acreage}</td>
															<td>${znnfMember.member_cultivate_position}</td>
															<td>${znnfMember.member_soil_fertility}</td>
															<td>${znnfMember.member_crop_historical_production}</td>
															<td>${znnfMember.member_fertilizer_costs}</td>
															<td>${znnfMember.member_comprehensive_cost}</td>
															<td>${znnfMember.member_remark}</td>
															<td>${znnfMember.member_update_name}</td>
															<td>${znnfMember.member_update_time}</td>
															<td>${znnfMember.commissioner_id}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMember/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMember/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMember/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMember/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
