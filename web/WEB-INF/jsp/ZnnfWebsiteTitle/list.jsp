						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfWebsiteTitle/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">TITLE_CONTENT</th>
														<th class="sorting_disabled">TITLE_OPERATE_TIME</th>
														<th class="sorting_disabled">TITLE_OPERATOR</th>
														<th class="sorting_disabled">TITLE_CATEGORY</th>
														<th class="sorting_disabled">TITLE_REMARK</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfWebsiteTitles}" var="znnfWebsiteTitle">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfWebsiteTitle.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfWebsiteTitle.title_content}</td>
															<td>${znnfWebsiteTitle.title_operate_time}</td>
															<td>${znnfWebsiteTitle.title_operator}</td>
															<td>${znnfWebsiteTitle.title_category}</td>
															<td>${znnfWebsiteTitle.title_remark}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteTitle/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteTitle/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteTitle/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteTitle/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
