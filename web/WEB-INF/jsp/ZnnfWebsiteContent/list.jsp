						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfWebsiteContent/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">CONTENT_INTRODUCE</th>
														<th class="sorting_disabled">CONTENT_IMAGE</th>
														<th class="sorting_disabled">CONNT_OPERATOR</th>
														<th class="sorting_disabled">CONTENT_OPERATE_TIME</th>
														<th class="sorting_disabled">CONTENT_REMARK</th>
														<th class="sorting_disabled">TITLE_ID</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfWebsiteContents}" var="znnfWebsiteContent">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfWebsiteContent.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfWebsiteContent.content_introduce}</td>
															<td>${znnfWebsiteContent.content_image}</td>
															<td>${znnfWebsiteContent.connt_operator}</td>
															<td>${znnfWebsiteContent.content_operate_time}</td>
															<td>${znnfWebsiteContent.content_remark}</td>
															<td>${znnfWebsiteContent.title_id}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteContent/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteContent/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteContent/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfWebsiteContent/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
