						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfLogin/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">LOGIN_NAME</th>
														<th class="sorting_disabled">LOGIN_TIME</th>
														<th class="sorting_disabled">LOGIN_IP</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfLogins}" var="znnfLogin">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfLogin.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfLogin.login_name}</td>
															<td>${znnfLogin.login_time}</td>
															<td>${znnfLogin.login_IP}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfLogin/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfLogin/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfLogin/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfLogin/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
