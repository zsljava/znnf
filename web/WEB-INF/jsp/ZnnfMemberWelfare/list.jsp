						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfMemberWelfare/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">WELFARE_RANK</th>
														<th class="sorting_disabled">WELFARE_INTEGRAL_LOWERLIMIT</th>
														<th class="sorting_disabled">WELFARE_INTEGRAL_UPPERLIMIT</th>
														<th class="sorting_disabled">WELFARE_DISCOUNT</th>
														<th class="sorting_disabled">WELFARE_REMARK</th>
														<th class="sorting_disabled">WELFARE_CREATE_NAME</th>
														<th class="sorting_disabled">WELFARE_CREATE_TIME</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfMemberWelfares}" var="znnfMemberWelfare">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfMemberWelfare.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfMemberWelfare.welfare_rank}</td>
															<td>${znnfMemberWelfare.welfare_integral_lowerlimit}</td>
															<td>${znnfMemberWelfare.welfare_integral_upperlimit}</td>
															<td>${znnfMemberWelfare.welfare_discount}</td>
															<td>${znnfMemberWelfare.welfare_remark}</td>
															<td>${znnfMemberWelfare.welfare_create_name}</td>
															<td>${znnfMemberWelfare.welfare_create_time}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMemberWelfare/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMemberWelfare/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMemberWelfare/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMemberWelfare/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
