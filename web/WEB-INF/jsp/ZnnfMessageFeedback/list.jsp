						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfMessageFeedback/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">FEEDBACK_TIME</th>
														<th class="sorting_disabled">SEND_ID</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfMessageFeedbacks}" var="znnfMessageFeedback">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfMessageFeedback.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfMessageFeedback.commissioner_number}</td>
															<td>${znnfMessageFeedback.feedback_time}</td>
															<td>${znnfMessageFeedback.send_id}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageFeedback/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageFeedback/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageFeedback/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfMessageFeedback/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
