						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/znnfCommissioner/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">COMMISSIONER_NUMBER</th>
														<th class="sorting_disabled">COMMISSIONER_PASSWORD</th>
														<th class="sorting_disabled">COMMISSIONER_NAME</th>
														<th class="sorting_disabled">COMMISSIONER_ID_NUMBER</th>
														<th class="sorting_disabled">COMMISSIONER_PHONE</th>
														<th class="sorting_disabled">COMMISSIONER_WORKPLACE</th>
														<th class="sorting_disabled">COMMISSIONER_ENTRYTIME</th>
														<th class="sorting_disabled">COMMISSIONER_STATUS</th>
														<th class="sorting_disabled">COMMISSIONER_REMARK</th>
														<th class="sorting_disabled">COMMISSIONER_CREATE_NAME</th>
														<th class="sorting_disabled">COMMISSIONER_CREATE_TIME</th>
														<th class="sorting_disabled">POSITION_ID</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${znnfCommissioners}" var="znnfCommissioner">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${znnfCommissioner.id}" class="ace" /> <span
															class="lbl"></span>
															</label></td>
															<td>${znnfCommissioner.commissioner_number}</td>
															<td>${znnfCommissioner.commissioner_password}</td>
															<td>${znnfCommissioner.commissioner_name}</td>
															<td>${znnfCommissioner.commissioner_id_number}</td>
															<td>${znnfCommissioner.commissioner_phone}</td>
															<td>${znnfCommissioner.commissioner_workplace}</td>
															<td>${znnfCommissioner.commissioner_entrytime}</td>
															<td>${znnfCommissioner.commissioner_status}</td>
															<td>${znnfCommissioner.commissioner_remark}</td>
															<td>${znnfCommissioner.commissioner_create_name}</td>
															<td>${znnfCommissioner.commissioner_create_time}</td>
															<td>${znnfCommissioner.position_id}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfCommissioner/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfCommissioner/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfCommissioner/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/znnfCommissioner/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
